...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        exec: {
            ...
            startPM2: {
                command: [
                    'pm2 start index.js --name handmaded'
                ].join('&&')
            },

            startPM2Clustering: {
                command: [
                    'pm2 start index.js --name handmaded -i max'
                ].join('&&')
            },

            startPM2Cron: {
                command: [
                    'pm2 start index.js --name handmaded -- -cron'
                ].join('&&')
            },

            startPM2ClusteringCron: {
                command: [
                    'pm2 start index.js --name handmaded -i max -- -cron'
                ].join('&&')
            },
